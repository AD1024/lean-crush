...
The representation deliberately follows the production encoding in
...
Keeping these generated declarations as total definitions is the first bridge
between the verified language and the live translator.  Symbol names and Lean
expression reification remain outside this core; their eventual refinement proof
must show that the production declarations have exactly these types.
-/
...
/-- Flatten all leading arrows in a source type.  This mirrors the production
...
/-! ## Declarations generated by the production defunctionalization scheme -/
...
`[Fn_(σ₁→σ₂→τ), ⟦σ₁⟧, ⟦σ₂⟧]` and result `⟦τ⟧`, exactly as the live pass does. -/
