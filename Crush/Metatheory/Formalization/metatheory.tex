...
The theorem justifies the reified higher-order sentence and the exact commands
...
\begin{quote}
If the exact raw commands returned by the proved VCG route are semantically
unsatisfiable, then the reified higher-order source sentence is unsatisfiable.
\end{quote}
...
theory, then constructs a relational SMT model satisfying the exact encoded
commands. Taking the contrapositive of those two constructions yields the
...
\begin{enumerate}[leftmargin=*,label=\arabic*.]
  ...
  \item Encode the typed target theory into raw SMT syntax and prove that every
        target model induces a model of the exact command sequence.
...
constructors represent source constants, application symbols, closures, and
certified primitives. Each constructor's result index is its exact declaration;
Section~4.2 spells out those four shapes.
...
\begin{enumerate}[leftmargin=*]
 ...
 \item a closure constructor
       ...
       \]
 \item a certified primitive symbol carrying a proof that its chosen semantic
       value equals the canonical flattened interpretation of its source
       constant.
\end{enumerate}
...
$\flattr{e}$ contains
\[
 \flattr{e}=(t_e,D_e,E_e,G_e,X_e,P_e),
\]
where:
\begin{itemize}[leftmargin=*]
 ...
 \item $G_e$ is the list of representation guards;
 \item $X_e$ is the list of extensionality formulas;
 \item $P_e$ is the list of certified primitive constraints.
\end{itemize}
The generated auxiliary theory and the complete theory of a source sentence are
\[
 \Gen(e)=E_e\mathbin{+\!+}G_e\mathbin{+\!+}X_e\mathbin{+\!+}P_e,
 \qquad
...
\begin{itemize}[leftmargin=*]
 ...
 \item certified primitives use their proved canonical interpretation.
\end{itemize}
...
\[
 ...
 \qquad\text{where }\flattr{e}=(t_e,D_e,E_e,G_e,X_e,P_e).
\]
...
spaces. Guards and primitive constraints are retained in the induction even
when a particular structural term generates none.
\end{proof}
...
computes $\flattr{\varphi}$ and then the exact command array
...
Guarded encodings and certified primitives are modeled components, but the
...
\begin{enumerate}[leftmargin=*]
 ...
 \item The pure encoder produces an exact ordered representation in raw SMT
...
\begin{enumerate}[leftmargin=*]
 ...
 \item \path{Flattened.generated_valid}: all generated equations, guards,
       extensionality formulas, and primitive constraints are valid.
 ...
 \item \path{SMT.commands_unsat_implies_theory_unsat}: semantic raw-command
       unsatisfiability reflects to typed FO unsatisfiability.
 \item \path{VCG.run_represents}: the fresh state returned by the total VCG
       contains the exact represented commands.
