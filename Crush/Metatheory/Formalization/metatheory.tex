...
\begin{itemize}[leftmargin=*]
 \item the inferred Lean type and the expected retained Lean type;
 \item exact correspondence of recognized expression and intrinsic-term shapes;
 \item the typed local-context bridge;
 \item the typed finite signature bridge for global constants.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
 ...
 \item The legacy direct, extensible \texttt{emitTerm} pipeline does not acquire
       ...
       certified.
 \item Unsupported syntax, dependent function types, datatypes in the proved
       raw-command fragment, unrestricted translation handlers, and native-HO
       commands are outside the present proved route.
\end{itemize}
...
\begin{description}[style=nextline]
 ...
 \item[3. Certify interpreted theories.]
 Extend the raw SMT semantics, encoding record, and model-lifting proof for
 interpreted integers and other selected built-ins. Compose guarded carrier
 encodings such as $\mathbb N\hookrightarrow\mathbb Z$ with those representations.

 \item[4. Expand structural coverage.]
 Add proof-facing datatype encodings, recursive datatype declarations, and
 selected value-indexed dependent forms. Each addition requires syntax,
 semantics, total reification, translation, and command representation---not
 only a new lowering rule.
...
\begin{enumerate}[leftmargin=*]
 ...
 \item \path{VCG.run_unsat_implies_source_unsat}: the direct composition
       for stateful VCG.
 \item \path{VCG.encoded_unsat_implies_source_unsat}: the specialization
       retaining a structural witness for the original Lean expression.
\end{enumerate}
