...
native datatype declarations, representation of source values by an existing
target type plus a predicate, and exact agreement with the final production
command array. It is written in dependency order:
...
\longrightarrow
\boxed{\text{untyped SMT command array}}
$}
\end{center}
...
\begin{quote}
If the represented final command array is semantically unsatisfiable, then
...
\begin{enumerate}[leftmargin=*,label=\arabic*.]
  ...
  \item Give semantics to the admitted untyped SMT command fragment and prove
        that the generated command array has exactly the intended models.
  \item Represent source values inside target types using predicates, validate
        native datatype and recursive guard commands, and combine all
        components in one SMT model.
  \item Retain structural Lean reification and compare the final production
        command array with the verified intrinsic command generator.
\end{enumerate}
...
\begin{array}{c@{\quad}l}
 ...
 C & \text{an ordered array of untyped SMT commands}.
\end{array}
\]
...
such proof calculus: intrinsic syntax is written as inhabitation of an indexed
...
An intrinsic source term is written $\HOTerm{\Sigma}{\Gamma}{e}{\tau}$.
...
\label{sec:datatypes}
\sectioncontents{Intrinsic mutual blocks; their embedding as HO base sorts and constants; finite free values; lawful model environments.}
...
declaration at position $d$. The corresponding intrinsic reference types are
\[
...
$q_1,\ldots,q_m$, its ownership witness selects an ordinary constant of type
\[
...
constant application. An unowned $\Base(\beta)$ remains completely opaque;
ownership by a datatype environment and the lawfulness condition below are
what give a selected base sort free-datatype meaning.
...
\emph{fields}: the modeled native SMT datatype representation has only external
base fields and recursive datatype fields, and the free-algebra proof is stated
...
sort $\mathsf{data}(d_{\Tree})$. Ownership selects ordinary HO constants with
types
\[
...
\begin{proposition}[FO image of datatype-owned source symbols]
Datatype membership does not change erasure: for every member $d$ of $B$,
\[
 \erase{\Base(\dsort{B}{d})}=\Base(\dsort{B}{d}).
\]
The curried HO type of an owned constructor is flattened to one FO
declaration
\[
 \bigl(
 ...
 \bigr).
\]
Owned selectors and testers similarly receive declarations
\[
 \begin{aligned}
  d_{\mathsf{sel},i}
...
the same source-constant identities selected by datatype ownership; only their
...
$\TreeFun$ for this function-value sort. The datatype-owned constructors have
...
concatenates them in this fixed order for semantic statements. The aggregate operation is Lean's
...
extensionality formula are different fields of \texttt{TranslationResult}:
...
The production SMT syntax is intentionally not intrinsically typed. From this
point onward, \emph{raw} means this untyped syntax, in contrast to the intrinsic
...
\sectioncontents{One shared encoding; semantic command-set representation; native datatype representation proofs; model lifting.}
...
\]
The first arrow records declaration structure and ownership; it is not a
denotational semantics for arbitrary \texttt{Lean.Expr}. The semantic bridge
begins with $\Lawful_B(S,M_s)$, which identifies the HO base carrier with the
canonical finite value family $\Val_B(M_s.B,d)$.

The operation component follows the same ownership map. An owned HO constructor,
...
indexed tester identifier recorded by $A$. The native
...
only through ownership, lawfulness, and the native representation proof.
\leanfiles{Reification/Datatype.lean, Datatype/Flattened.lean, SMT/Datatype.lean, and SMT/DatatypeRepresentation.lean}
\end{definition}

\begin{example}[The native tree declaration]
For the later guarded specialization in which source $\Num$ values are
represented by SMT integers, the allocated native command may be printed as
...
implicit testers such as \texttt{(\_ is leaf)}. Its native datatype laws say
...
identifiers owned by $\mathcal E$.
...
Production can represent a source type using an existing SMT type together
...
\tag{G}
\]
The production-shaped predicate expresses the same condition without pattern
matching:
\[
\begin{aligned}
...
\tag{PG}
\]
Native constructor exhaustiveness and tester disjointness make (G) and (PG)
equivalent. This is the concrete content of
\texttt{Val.wf\_iff\_selWF}; it is why a native SMT datatype carrier can be
...
each step, the current block receives its free-algebra type and native
interpretations. An \texttt{Ordered} proof states that every earlier native
...
the external representation information for \texttt{Nat}, and the intrinsic
...
function that turns the reified finite theory into the command array whose
...
\[
\begin{array}{c@{\quad}l}
 C_{\mathsf{native}} & \text{the native \texttt{Tree} declaration},\\
...
produced: it proves command-set equivalence with the guarded encoder's output.
The guarded model-lifting theorem then constructs one SMT model in which the
encoded formulas from (R), the native declaration, and the recursive guard
...
representation above is composed with native datatype and guard validity by
the production-facing theorem in the next section.
\leanfile{VCG/Generate.lean}
\end{theorem}

\section{Comparing the final production commands with the intrinsic theory}
\sectioncontents{Command locations; representation proofs; checked final-array comparison; reflection.}

The intrinsic VCG constructs a canonical command array. The production
...
\subsection{Locating datatype commands in the final array}

\begin{definition}[Production fact]
A \texttt{ProductionFact} retains:
\begin{itemize}[leftmargin=*]
 \item the source expression, its datatype environment, its ordinary signature
       and constants, and the intrinsic sentence when the expression is in the
       supported fragment;
 \item the complete final production command array $C_{\mathsf{prod}}$;
 \item the positions of its native datatype commands in that array; and
 \item the positions of its recursive guard definitions in that array.
\end{itemize}
Each recorded position comes with a Lean proof that the command at that position
belongs to the corresponding intrinsically typed datatype block. These
positions are recomputed after all facts are translated, so every
\texttt{ProductionFact} refers to the same final command array rather than an
intermediate prefix.
\leanfile{VCG/Datatype.lean}
\end{definition}

\begin{definition}[Production representation evidence]
For a production fact $P$:
\begin{itemize}[leftmargin=*]
 \item $P.\mathsf{Representation}(\mathcal E)$ links every located native
       command to one shared encoding and proves that the encoding's native
...
\end{itemize}
These records connect production commands to the already-proved intrinsic
...
$\Phi_W$. A \texttt{ProductionTheoryAgreement} is the proposition
\[
 \Rep_g(\mathcal G,C_{\mathsf{guard}},\Theorys{\Phi_W},
   \mathsf{strip}(C_{\mathsf{prod}})).
\]
The executable \texttt{ProductionTheoryAgreement.build?} computes the expected
guarded commands and checks inclusion in both directions. On success it returns
both the comparison result and its Lean proof. Thus reordering and duplicate
removal are accepted, while a missing or extra semantically relevant command is
rejected.
\leanfile{VCG/Production.lean}
\end{definition}

\begin{example}[What final-array agreement checks]
Suppose production emits the running commands in a different legal order,
...
it contains the native \texttt{Tree} declaration, \texttt{wf\_Tree}, every
...
Let $P$ be a production fact with native representation, static guard
representation, uniform guard interpretation, and whole-theory agreement for
$W$. If the final production command array is semantically unsatisfiable, then
\[
 \unsat_{\mathsf{SMT}}(C_{\mathsf{prod}})
 \quad\Longrightarrow\quad
 \unsat_{\mathsf{HO}}^{\mathcal D}(\Phi_W).
\]
The theorem \texttt{ProductionTheoryAgreement.unsat\_source\_script} also admits the
leading logic-selection command returned by \texttt{buildScript}.
\texttt{ProductionFactAgreement} is the single-fact specialization
...
representation witness. The production translator's extensible \texttt{emitTerm} route is
marked trusted at its root, and unrestricted handlers, constants or closures
without semantic proofs, and native-HO emission retain additional reasons.
Individually proved components become a whole-run theorem only through the exact
aggregate agreement described above.
\leanpaths{Crush/Metatheory/VCG/Trust.lean, Crush/Translation/Monad.lean, and Crush/Metatheory/VCG/Production.lean}
...
\begin{tikzpicture}[
  ...
  \node[stage,right=of fo] (spec) {verified guarded\\command array};
  \node[boundary,right=of spec] (prod) {final production\\command array $C$};
...
agreement models production reordering and duplicate suppression exactly in the
...
\begin{enumerate}[leftmargin=*]
 ...
 \item Productive monomorphic datatype blocks have finite free-algebra source
       semantics and exact native command semantics in the untyped SMT model.
 ...
 \item One induced untyped SMT model validates native datatype commands, simultaneous
 ...
 \item Common-environment reification retains the exact ordered Lean fact array,
       and the pure finite-theory VCG proves the meaning of its command array.
 \item Command locations reconnect native datatype and recursive guard commands
       to the final production array. \texttt{ProductionTheoryAgreement.build?}
       checks the rest of that array and returns a proof of agreement.
 \item Semantic unsatisfiability of the agreed final command array reflects to
...
\begin{itemize}[leftmargin=*]
 ...
 \item The extensible production translator does not automatically construct the
 ...
 \item \texttt{TranslateState.status} is operational metadata; it cannot replace
       a \texttt{TheoryRepresentation} or \texttt{ProductionTheoryAgreement} witness.
 \item The production allocator is finite, while the intrinsic \texttt{SMT.Encoding}
 ...
 \item A ground production type such as \texttt{Tree (A -> A)} is not accepted
       as a native datatype. The legacy acceptance check rejects the arrow-valued
       field, and certified reification reports \texttt{functionField}. Production
       ...
       none of the free-datatype laws and does not produce the represented native
       datatype block required by the production-agreement theorem.
...
\begin{enumerate}[leftmargin=*]
 ...
 \item \path{SMT.commands_unsat_implies_source_theory_unsat}: semantic
       untyped-command unsatisfiability reflects to the lawful intrinsic HO theory.
 ...
 \item \path{VCG.theoryCommands_unsat_implies_source_unsat}: unsatisfiability
       of the pure finite-theory command array reflects to source-theory
       unsatisfiability.
 \item \path{VCG.ProductionFact.Representation.theory_unsat}: one production
       datatype and guard package reflects any exactly represented finite
       source theory.
 \item \path{VCG.ProductionTheoryAgreement.unsat_source} and
       \path{VCG.ProductionTheoryAgreement.unsat_source_script}: the final
       production command array, with an optional leading logic command,
