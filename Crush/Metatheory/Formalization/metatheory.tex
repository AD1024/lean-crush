...
\texttt{VCG.TranslationStatus} makes the boundary structural. A
\texttt{proved} result stores the intrinsic sentence, exact commands, and their
representation theorem. A \texttt{trusted} result stores commands and a
nonempty list of trust reasons but exposes no theorem for reflecting command
unsatisfiability. The legacy extensible \texttt{emitTerm} route is marked trusted
at its root; unrestricted handlers, uncertified constants or closures, and
native-HO emission add more specific reasons.
\leanfiles{VCG/Trust.lean and VCG/Status.lean}
